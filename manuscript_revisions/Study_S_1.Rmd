...
df3 <- read.csv("../data/study_S1_data_long.csv")
df1 <- read.csv("../data/study_S1_data_wide.csv")
x <- read.csv("../data/study_S1_data_long_clean.csv")
...
The design, materials, and procedure for Study S1 were the same as for Study 2, the only change from Study 2 was that all participants in Study S2 were recruited from MTurk. Study S1 was a within-subjects design. The independent variable was condition with two levels, diagnostic and non-diagnostic. We used the same two dependent variables as in previous studies, the four item moral perception scale (MPS-4, $\alpha$ = `r round(alpha1$alpha,digits=2)`), and the single item moral perception measure MM-1.
...
```{r}
df1 <- read.csv("../data/study_S1_data_long.csv")
df3 <- read.csv("../data/study_S1_data_long_clean.csv")
df_long_clean <- df3
table(df3$gender)
length(df1$gender)/6
...
*Robin*,
*M*~MPS-4~ = `r mean(robin$R_tot)`, *SD*~MPS-4~ = `r sd(robin$R_tot)`. There was significant variation depending on the description, *F*(`r round(aov1$DFn)`,`r round(aov1$DFd)`) = `r aov1$F`, *p* `r paste(p_report(aov1$p))`, partial $\eta$^2^ = `r paste(round(aov1$ges,digits=3))`. Pairwise comparisons did not reveal any significant differences between individual descriptions (all *p*s > .05).
...
supp_s1_rtot_stats <- s_pooled_fun()
d <- supp_s1_rtot_stats$d
...
supp_s1_rtot_vars <- as.data.frame(VarCorr(model1))
...
Overall, the model significantly predicted participants responses, and provided a better fit for the data than the baseline model, $\chi$^2^(`r results_anova$Df[2]`) = `r results_anova$Chisq[2]`, *p* `r paste(p_report(p1))`. Condition did not influence responses to the MPS-4, *F*(`r aov1$NumDF[1]`, `r aov1$DenDF[1]`) = `r f3`, *p* `r paste(p_report(p3))`; and was not a significant predictor in the model when controlling for scenario, $b$ = `r results_coef$Estimate[2]`, *t*(`r round(results_coef$df[2])`) = `r t1`, *p* `r paste(p_report(p2))`, (*d* = `r round(d, digits=2)`), see Figure\ \@ref(fig:S4bothconditionplot).
...
```{r S4bothconditionplot, fig.cap="Study S1: Responses to moral perception measures depending on condition", include=TRUE, out.width = "\\textwidth", fig.pos = "!h"}
...
supp_s1_M1_stats <- s_pooled_fun()
d <- supp_s1_M1_stats$d
d

save(supp_s1_rtot_stats, supp_s1_M1_stats, file = "supp_S1_effect_stats.RData")
...
supp_s1_M1_vars <- as.data.frame(VarCorr(model1))


save(supp_s1_rtot_for_table, supp_s1_M1_for_table, supp_s1_M1_vars, supp_s1_rtot_vars, file = "supp_S1_table_data.RData")


```

We conducted a linear-mixed-effects model to test if condition influenced MM-1 responses. Our outcome measure was MM-1, our predictor variable was condition; we allowed intercepts and the effect of condition to vary across participants. Overall, the model significantly predicted participants responses, and provided a better fit for the data than the baseline model, $\chi$^2^(`r results_anova$Df[2]`) = `r results_anova$Chisq[2]`, *p* `r paste(p_report(p1))`. Condition significantly influenced MM-1 responses *F*(`r round(aov1$NumDF[1])`, `r round(aov1$DenDF[1])`) = `r f3`, *p* `r paste(p_report(p3))`, and was a significant predictor in the model $b$ = `r results_coef$Estimate[2]`, *t*(`r round(results_coef$df[2])`) = `r t1`, *p* `r paste(p_report(p2))`, (*d* = `r round(d, digits=2)`), see Figure\ \@ref(fig:S4bothconditionplot).


### Study S1: Combined Measure
...
*Robin*,
*M* = `r mean(robin$M1R_tot)`, *SD* = `r sd(robin$M1R_tot)`. There was significant variation depending on the description, *F*(`r round(aov1$DFn)`,`r round(aov1$DFd)`) = `r aov1$F`, *p* `r paste(p_report(aov1$p))`, partial $\eta$^2^ = `r aov1$ges`. Follow-up pairwise comparisons did not reveal any significant differences between the different characters (all *p*s > .05). 
...
### Study S1: Differences between the Descriptions
...
```{r Supp1AllscenariosMPS4plot, fig.cap="Study 2: Differences in MPS-4 depending on condition", include=FALSE}
...
```{r Supp1AllscenariosM1plot, fig.cap="Study 2: Differences in MPS-4 depending on condition", include=FALSE}
...
```{r Supp1AllscenariosBothplot, fig.cap="Study 2: Differences in MPS-4 depending on condition", include=FALSE}
...
```{r Supp1AllscenariosPlot, fig.cap="Study 2: Differences in moral perception for each description", include=TRUE, fig.height=8, out.width = "\\textwidth", fig.pos = "!h"}
